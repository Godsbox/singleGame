...
public abstract class ContentFragment<P extends Presenter>
    ...
    @Override
    public void showLoadFailure(String errorMessage) {
    ...
    public void showLoadEmpty() {
        if (contentConfig != null) {
            loadView.setEmpty(contentConfig.emptyTitle, contentConfig.emptyMessage,
                    contentConfig.emptyImage, contentConfig.emptyAction);
            loadView.setNeedLoginView(contentConfig.needLogin && !getPresenter().isLogin());
        }
        loadView.showEmpty(getLoadViewContainer());
    }

    @Override
    public void showLoadEmptySecondary(String title) {
        ContentConfig contentConfig = contentConfigSecondary == null
                ? this.contentConfig : contentConfigSecondary;
        if (contentConfig != null) {
            loadView.setEmpty(title, contentConfig.emptyMessage,
                    contentConfig.emptyImage, contentConfig.emptyAction);
            loadView.setNeedLoginView(contentConfig.needLogin && !getPresenter().isLogin());
        }
        loadView.showEmpty(getLoadViewContainer());
