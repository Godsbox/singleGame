...
import static com.lzj.arch.R.id.go;
...
//        if (layoutId == R.layout.app_view_load_small) {
//            height = WRAP_CONTENT;
//        }
    ...
    private void show(View view) {
        ...
        if (parent instanceof LinearLayout) {
            parent.addView(this, id == load_view_container ? 1 : 0);
            return;
        }
        parent.addView(this);
    }
     ...
     * 设置空页面提示。
     *
     * @param title   提示语资源 ID
     * @param message 消息资源 ID
     * @param image   提示图片资源
     * @param action  操作文案 ID
     */
    void setEmpty(@StringRes int title, @StringRes int message, @DrawableRes int image, @StringRes int action) {
        if (title > 0) {
     ...
     * @param title 标题
     */
    void setEmpty(String title, @StringRes int message, @DrawableRes int image, @StringRes int action) {
        setEmpty(0, message, image, action);
        emptyTitle.setText(title);
    }
    ...
    public void setErrorMessage(String error) {
        ...
        ViewUtils.setGone(reload, !error.contains("评论不存在"));
    }
